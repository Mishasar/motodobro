...
$num = $r['num'];
            ...
            $message  = "Запрос счета по заказу #".$num." от ".strftime("%d.%m.%Y", strtotime($r['t_date']))."\n\n";
            $message .= "Дата запроса: ".date("d.m.Y H:i:s")." \n";
            $message .= "Имя: " . $r['full_name'] . "\n";
			$message .= "Телефон: " . $r['phone'] . "\n";
			$message .= "E-mail: " . $r['email'] . "\n";
            $message .= "Сумма заказа: " .number_format($r['sum_order_rub'], 2, '.', ' '). "\n\n";			
			
			mail($site_mail, $site_name.": Запрос счета по заказу #".$num, $message, $mail_header);
