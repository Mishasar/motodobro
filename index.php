...
if (isset($_REQUEST['state'])):
   ...
   if ($url_array[1] == "supplier"):
		...
		$state = $_lnk[0];
                $state_value = count($_lnk) > 1 ? $_lnk[1] : '';
   endif;
endif;
...
switch ($state) {
    ...
    case "preordernoreg":
        include('modules/card.mod'); break;
    case "orderpay":
        include ('modules/orderpay.mod'); break;
    case "orderpaynoreg":
        include ('modules/orderpaynoreg.mod'); break;
    ...
    case "basket":
		if (isset($_REQUEST['face']) && $_REQUEST['face']!=''){
                    include ('modules/register.mod'); break;
		}else{
...
// меню OEM запчасти
	$oem_menu = '<ul class="dropdown-menu pull-right" role="menu">';
        //$itemsOEMVehicle = $db->getOEMVehicleItems();        
        $s = sql("SELECT id, name, prim FROM w_group ORDER BY ord");
	while ($r = mysql_fetch_array($s)):
        //foreach ($itemsOEMVehicle as $key => $vehicle):
            $oem_menu .= '<li class="dropdown-submenu">
                            <a tabindex="-1" href="#">'./*$vehicle['name']*/ $r['name'].'</a>
                            <ul class="dropdown-menu pull-left">';
        /*
                $OEMVehicleBrands = $db->getOEMVehicleBrandsByItem($vehicle['id']);
                foreach ($OEMVehicleBrands as $k => $brand):
                    $url = $site_url.'/1_'.$brand['id'].'_'.encodestring($vehicle['prim'].' '.$brand['name']).'.html';
                    $oem_menu .= '<li><a tabindex="-1" href="'.$url.'" style="text-align:left;">'.$brand['name'].'</a></li>';
                endforeach;	
                */
...
// меню Аксессуары запчасти
	$accesories_menu = '<ul class="dropdown-menu pull-right" role="menu">';
	$s = sql("SELECT id, name  FROM w_un_brand WHERE tpl=3 ORDER BY name");
	while ($r = mysql_fetch_array($s)):
		$accesories_menu .= '<li class="dropdown-submenu">
		...
		while ($r2 = mysql_fetch_array($s2)):						
			...
			$accesories_menu .= '<li><a tabindex="-1" href="'.$url.'" style="text-align:left;">'.$r2['name'].'</a></li>';
		endwhile;	
		$accesories_menu .= '</ul>
					</li>';				
	endwhile;
...
$tpl->assign(array('TPL_MAIN'   => $tpl_main,
                    ...
                    'count_cart_items'  => number_format($r[0], 0),
                    'OEM_MENU' => $oem_menu,
                    'ACC_MENU' => $accesories_menu,
                    'GOROD' => $gorode,
                    'srh_txt' => ''
				));
