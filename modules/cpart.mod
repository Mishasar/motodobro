<?php
  ...
  $s = sql("SELECT g.prim as g_name, b.name as b_name, m.name as model_name, my.name as year, u.name_rus, u.name,
                   u.id, u.year_id, u.model_id, u.brand_id, u.group_id, g.id as gid, b.pr_group, p.code, p.descr, p.descr_rus, u.img_path, u.img_big
  ...
  $h3 = strtoupper($r['code'].' '.$part_name);
	...
	$navigate = '<li><a href="'.$site_url.'/">Главная</a> <span class="divider">/</span></li>
	  <li><a href="'.$site_url.'/1_'.$r['brand_id'].'_'.encodestring($r['g_name'].' '.$r['b_name']).'.html">'.$r['g_name'].' '.$r['b_name'].'</a> <span class="divider">/</span></li>
...
if ($user_id > 0): 
  ...
  $r = mysql_fetch_array($s);
  $cl_group = $r['group_id'];
else: $cl_group = 1; endif;

// поиск формулы расчета цены
  $s = sql("SELECT value FROM price p WHERE p.client_gr=$cl_group AND p.prod_gr=$pr_group");
  $r = mysql_fetch_array($s);
  $sr_formula = $r['value'];
         ...
         if ($code_ref != ''):
          ...
          if ($r['qty_unit'] > 1 ):
            ...
            $list_model .= '					
                    ...
                    <tr><td ><span class="tbl_lable">Цена за шт</span>: <b>'.$dis_price.'</b></td></tr>';
  ...
  $list_model .= '</tr>';
         ...
         endwhile;
            $new_tlt_srh_count = '<div class="alert alert-black">Деталь с кодом <u>'.$code.'</u> найдена в '.$cat_count.' каталогах оригинальных запчастей:</div>';
		 else:
