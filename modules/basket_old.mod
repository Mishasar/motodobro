...
if (mysql_num_rows($s)>0):
  while ($r = mysql_fetch_array($s)):
     ...
     if ($j==2): $bgcolor = ' bgcolor="#48535b"'; $j =0; else: $bgcolor=' bgcolor="#37434a"'; endif;
        $tpl -> assign( array(id    => $r['prod_id'],
							  ...
							  qty     => '<input class="span2 qty_item" size="16" type="number" value="'.$r['qty'].'" name="array_am['.$i.'][qty]" style="width:56px;">
                                           <input type="hidden" name="array_am['.$i.'][id]" value="'.$r['prod_id'].'">',
                              sum    => number_format($sum, 2, '.', ' '),
                              del => '<a href="'.$site_url.'/basket.html?delete='.$r['prod_id'].'" data-toggle="tooltip" title="удалить товар из корзины"><img src="'.$site_url.'/img/cross.png" height="24" width="24" hspace="0" vspace="0" ></a>'
                              ));
  ...
  $prim = '';
  $s2 = sql("SELECT * FROM site_template WHERE id=1");
