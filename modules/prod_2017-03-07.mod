...
function russian_date($d){
  ...
  $s = sql("SELECT p.code, p.descr, p.descr_rus FROM wn_parts p, w_brand b WHERE p.brand_id=b.id AND b.name='$supplier' AND p.code='$code'");
  $r = mysql_fetch_array($s);
  if ($r['code'] != '') {
  ...
  $s = sql("SELECT d.id, code, descr, descr_rus, code_ref, pr1, pr2, curr, group_id FROM detali d, price_group g WHERE d.group_id=g.id AND (g.name='$supplier' OR g.id=10) AND d.code='$code'");
  $r = mysql_fetch_array($s);
  if ($r['code'] != '') {
  ...
  $navigate = '<li><a href="'.$site_url.'/">Главная</a> <span class="divider">/</span></li><li class="active">'.$supplier.' <span class="divider">/</span></li><li class="active">'.$code.'</li>';
  $h1 = $code;
  $h3 = $descr . ' ' . $supplier;
  ...
  $card_code = $code .'<br>'.$descr.'<br>'.$supplier;
			...
			if ($r3['qty'] > 0) {
				...
				$nalich = 'Под заказ';  
				$deliv_time = 'приблизительно '.russian_date( date('d.m.Y', strtotime ('+14 day')) ).'...'.russian_date( date('d.m.Y', strtotime ('+21 day')) );
		...
		$list_model = '<tr ><td style="border-top:0px;">Номер в каталоге</span>:';
         if ($code_ref != ''):
		...
		if ($descr_rus != '') { 
			$list_model .= '
                    <tr><td ><span class="tbl_lable">Описание</span>: '.$descr_rus.'</td></tr>';
		}
		if ($descr_en != '') {	
			$list_model .= '
                    <tr><td ><span class="tbl_lable">Описание ориг.</span>: '.$descr_en.'<br><i>'.$note.'</i></td></tr>';	
		}			
		if ($id > 0) {			
			$list_model .= '					                    
					<tr><td ><span class="tbl_lable">Состояние</span>: Новая оригинальная запчасть</td></tr>
					<tr><td ><span class="tbl_lable">Наличие</span>: '.$nalich.'</td></tr>
					<tr><td ><span class="tbl_lable">Доставка</span>: '.$deliv_time.'</td></tr>
                    <tr><td ><span class="tbl_lable">Цена</span>: <span style="font: 17px \'segoe_ui_light\',sans-serif;font-weight:bold;">'.$dis_price.'</span></td></tr>';
