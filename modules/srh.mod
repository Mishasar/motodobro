...
if ($srh_txt != '' && strlen($srh_txt) > 3):
    ...
    $s = sql("SELECT DISTINCT d.code, g.name  FROM detali d, price_group g WHERE g.id=d.group_id AND g.id<>10 AND (d.code LIKE '$lbl_srh%' OR  d.code_alt LIKE '$srh_txt%')
				UNION 
				SELECT DISTINCT p.code, b.name_prc AS name  FROM wn_parts p, w_brand b WHERE b.id=p.brand_id AND (p.code LIKE '$lbl_srh%' OR p.code_alt LIKE '$srh_txt%')
				LIMIT 50");
    ...
    if ($res > 0):
        if ($res == 1):
            ...
            header('Location: '.$site_url. '/supplier/'.urlencode($supplier).'/'.urlencode($code).'.html');
            exit;
            ...
            */			
            ...
            while ($r = mysql_fetch_array($s)):
                $code = $r['code'];
                $supplier = $r['name'];
                $code_alt = str_replace('-', '', $code);
                $code_alt = str_replace('.', '', $code_alt);
                
                $s2 = sql("SELECT p.code, p.descr, p.descr_rus FROM wn_parts p, w_brand b WHERE p.brand_id=b.id AND ( b.name='$supplier' OR b.name_prc='$supplier' ) AND ( p.code='$code' OR p.code_alt='$code')");
                ...
                $s2 = sql("SELECT d.id, code, descr, descr_rus, code_ref, pr1, pr2, curr, group_id FROM detali d, price_group g WHERE d.group_id=g.id AND (g.name='$supplier' OR g.id=10) AND (d.code='$code' OR d.code='$code_alt')");
                $r2 = mysql_fetch_array($s2);
                $code_ref = '';
                if ($r2['code'] != '') {
        ...
        $tlt_srh = '<div class="alert alert-block">По запросу <u>'.$lbl_srh.'</u> ничего не найдено!<br> Напишите нам запрос, поможем с подбором. <br> <script>var amo_forms_params = {"id":378754,"hash":"5a4fa588c6ed479ee3d7adb2ed24403e","locale":"ru"};</script><script id="amoforms_script" async="async" charset="utf-8" src="https://forms.amocrm.ru/forms/assets/js/amoforms.js"></script></div>';
...
if ($printPage):     
    $tpl -> assign( array('tlt_srh' => $tlt_srh, 'new_tlt_srh_count' => $new_tlt_srh_count));
