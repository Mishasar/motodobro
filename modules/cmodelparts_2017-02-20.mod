		...
		if ($id > 0) {
			...
			if ($dis_price <> '--') { 
				$list_model .= '
					...
					<input class="span2" id="appendedPrependedInput" type="number" value="'.$r['qty_unit'].'" name="qty" style="width:40px;">
					<button class="btn" style="height:30px;" type="submit" name="add" onclick="addProduct(\'#product'.$id.'\');">В корзину</button>
					</div>
					*/
				$list_model .= '	
					<button class="btn btn-sm" style="height:30px;" type="submit" name="add" onclick="addProduct(\'#product'.$id.'\');">Купить</button>
			...
			if ($r['code'] != '') {
				...
				$list_model .= '<br><small><i>'.$descr.'</i></small></td><td>'.$r['qty_unit'].'</td><td nowrap colspan="2">---<br><span class="badge badge-inverse">Запросить цену</span></td></tr>';		
			}	
		}      
